		...
		for (ChunkPos chunkcoordintpair : eligibleChunksForSpawning) {
			...
			if (world.rand.nextDouble() < Config.getSpawnChance()) {
				...
				if (spawner != null) {
					...
					int y = spawner.getY();
					int z = spawner.getZ();
					Biome biome = world.getBiomeGenForCoords(spawner);

					if ((Config.getBiomeWhitelist().isEmpty() && BiomeDictionary.isBiomeOfType(biome, BiomeDictionary.Type.HOT) && BiomeDictionary.isBiomeOfType(biome, BiomeDictionary.Type.DRY)) || Config.getBiomeWhitelist().contains(biome.getRegistryName())) {
