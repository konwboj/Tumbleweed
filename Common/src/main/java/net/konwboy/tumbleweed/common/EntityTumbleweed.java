...
public class EntityTumbleweed extends Entity {
	...
	protected void dropFromLootTable(DamageSource damageSource) {
		LootTable loottable = this.level().getServer().getLootData().getLootTable(getType().getDefaultLootTable());
		loottable.getRandomItems(createLootContext(damageSource)).forEach(this::spawnAtLocation);
	}

	protected LootParams createLootContext(DamageSource damageSource) {
