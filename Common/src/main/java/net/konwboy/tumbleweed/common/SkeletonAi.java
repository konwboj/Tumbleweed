...
public class SkeletonAi {
	...
	public static void modifySkeleton(AbstractSkeleton skeleton) {
		var goalSelector = ((MobAccessor)skeleton).getGoalSelector();

		if (goalSelector.getAvailableGoals().stream().noneMatch(g -> g.getGoal().getClass() == ShootTumbleweed.class))
			goalSelector.addGoal(5, new ShootTumbleweed(skeleton));
	}

	static class ShootTumbleweed extends Goal {
